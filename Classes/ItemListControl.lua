...
local ItemListClass = newClass("ItemListControl", "ListControl", function(self, anchor, x, y, width, height, itemsTab, forceTooltip)
	...
	self.controls.deleteUnused = new("ButtonControl", {"RIGHT",self.controls.deleteAll,"LEFT"}, -4, 0, 70, 18, "清理未使用", function()
		local delList = {}
	...
	self.controls.sort = new("ButtonControl", {"RIGHT",self.controls.deleteUnused,"LEFT"}, -4, 0, 60, 18, "排序", function()
...
function ItemListClass:OnSelDelete(index, itemId)
	...
	if equipSlot then
		...
		main:OpenConfirmPopup("删除装备", item.name.." 已经装备在了 "..equipSlot.label..inSet..".\n你确定要删除吗?", "删除", function()
		...
		if equipSet then
			local inSet = equipSet and (" in set '"..(equipSet.title or "默认套装").."'") or ""
			main:OpenConfirmPopup("删除装备", item.name.." 在天赋树 '"..equipTree.."'中被使用.\n你确定要删除吗?", "删除", function()
				self.itemsTab:DeleteItem(item)
				self.selIndex = nil
				self.selValue = nil
			end)
