...
local catalystQualityFormat = {
	...
	"^x7F7F7FQuality (速度词缀): "..colorCodes.MAGIC.."+%d%% (augmented)",
	...
	"^x7F7F7FQuality (物理和混沌词缀): "..colorCodes.MAGIC.."+%d%% (augmented)",
	...
	"^x7F7F7FQuality (暴击词缀): "..colorCodes.MAGIC.."+%d%% (augmented)",
}
...
self.controls.slotHeader = new("LabelControl", {"BOTTOMLEFT",self.slotAnchor,"TOPLEFT"}, 0, -4, 0, 16, "^7已装备:")
	...
	if main.portraitMode then
		self.controls.itemList = new("ItemListControl", {"TOPRIGHT",self.controls.specButton,"BOTTOMRIGHT"}, 0, 20, 360, 308, self)
	else
		self.controls.itemList = new("ItemListControl", {"TOPLEFT",self.controls.setManage,"TOPRIGHT"}, 20, 20, 360, 308, self)
	end
	-- Database selector
	...
	-- Create/import item
	self.controls.craftDisplayItem = new("ButtonControl", {"TOPLEFT",main.portraitMode and self.controls.setManage or self.controls.itemList,"TOPRIGHT"}, 20, main.portraitMode and 0 or -20, 120, 20, "创建装备...", function()
...
如果有2个位置都可以装备那件物品，那么想快捷装备到第二个位置，那么多按一个Shift键。]])
	...
	self.controls.displayItemEnchant = new("ButtonControl", {"TOPLEFT",self.controls.displayItemSectionEnchant,"TOPLEFT"}, 0, 0, 100, 20, "附魔", function()
	...
	self.controls.displayItemEnchant2 = new("ButtonControl", {"TOPLEFT",self.controls.displayItemEnchant,"TOPRIGHT",true}, 8, 0, 160, 20, "附魔2", function()
	...
	self.controls.displayItemAnoint = new("ButtonControl", {"TOPLEFT",self.controls.displayItemEnchant2,"TOPRIGHT",true}, 8, 0, 100, 20, "涂油", function()
	...
	self.controls.displayItemAnoint2 = new("ButtonControl", {"TOPLEFT",self.controls.displayItemAnoint,"TOPRIGHT",true}, 8, 0, 100, 20, "涂油 2", function()
	...
	self.controls.displayItemAnoint3 = new("ButtonControl", {"TOPLEFT",self.controls.displayItemAnoint2,"TOPRIGHT",true}, 8, 0, 100, 20, "涂油 3", function()
	...
	self.controls.displayItemAnoint4 = new("ButtonControl", {"TOPLEFT",self.controls.displayItemAnoint3,"TOPRIGHT",true}, 8, 0, 100, 20, "涂油 4", function()
	...
	self.controls.displayItemCorrupt = new("ButtonControl", {"TOPLEFT",self.controls.displayItemAnoint4,"TOPRIGHT",true}, 8, 10, 100, 20, "腐化", function()
	...
	--[[
	self.controls.displayItemScourge = new("ButtonControl", {"TOPLEFT",self.controls.displayItemCorrupt,"TOPRIGHT",true}, 8, 0, 100, 20, "灾魇腐化", function()
	...
	self.controls.displayItemAddImplicit.shown = function()
		return self.displayItem and 
			(self.displayItem.corruptable or ((self.displayItem.type ~= "Flask" or self.displayItem.type ~= "Jewel") and 
				(self.displayItem.rarity == "NORMAL" or self.displayItem.rarity == "普通" or 
				self.displayItem.rarity == "MAGIC" or self.displayItem.rarity == "魔法" or 
				self.displayItem.rarity == "RARE" or self.displayItem.rarity == "稀有")))
	end
	
	-- Section: Influcence dropdowns
	local influenceDisplayList = { "影响效果" }
		...
		drop.tooltipFunc = function(tooltip, mode, index, value)
			...
			elseif tooltip:CheckForUpdate(modList) then
				...
				if node then
					...
					-- Comparison
					tooltip:AddSeparator(14)
					self:AppendAnointTooltip(tooltip, node, "配置")
...
function ItemsTabClass:Draw(viewPort, inputEvents)
	...
	for index, itemSetId in ipairs(self.itemSetOrderList) do
		local itemSet = self.itemSets[itemSetId]
		t_insert(newItemList, itemSet.title or "默认套装")
...
function ItemsTabClass:EnchantDisplayItem(enchantSlot)
	...
	controls.enchantmentSourceLabel = new("LabelControl", {"TOPRIGHT",nil,"TOPLEFT"}, 95, 45, 0, 16, "^7迷宫:")
	...
	if item then
		for _, modList in ipairs{item.enchantModLines, item.scourgeModLines, item.implicitModLines, item.explicitModLines} do
			for _, mod in ipairs(modList) do
				local line = mod.line
				local anoint = line:find("配置 (.+)")
				if anoint then
					local nodeName = line:sub(anoint + string.len("配置 "))
...
function ItemsTabClass:anointItem(node)
	...
	if node then
		t_insert(item.enchantModLines, self.anointEnchantSlot, { crafted = true, line = "配置 " .. node.dn })
	end
...
function ItemsTabClass:AnointDisplayItem(enchantSlot)
	...
	local function saveLabel()
		...
		if curAnoints and #curAnoints >= self.anointEnchantSlot then
			return "移除 "..curAnoints[self.anointEnchantSlot]
		end
		return "无涂油"
	end
	...
	controls.close = new("ButtonControl", {"TOPLEFT", controls.save, "TOPRIGHT" }, 10, 0, 80, 20, "取消", function()
		main:ClosePopup()
	end)
...
function ItemsTabClass:CorruptDisplayItem(modType)
	...
	controls.source = new("DropDownControl", {"TOPLEFT",nil,"TOPLEFT"}, 100, 20, 150, 18, sourceList, function(index, value)
		local modTypeEN = { 腐化 = "Corrupted", 灾魇腐化 = "Scourge" }
		value = modTypeEN[value]
	...
	controls.implicitLabel = new("LabelControl", {"TOPRIGHT",nil,"TOPLEFT"}, 75, 45, 0, 16, "^7基底#1:")
	...
	controls.implicit2Label = new("LabelControl", {"TOPRIGHT",nil,"TOPLEFT"}, 75, 65, 0, 16, "^7基底#2:")
	...
	controls.implicit3Label = new("LabelControl", {"TOPRIGHT",nil,"TOPLEFT"}, 75, 85, 0, 16, "^7基底#3:")
	...
	controls.implicit3Label.shown = false
	controls.implicit3.shown = false
	controls.implicit4Label = new("LabelControl", {"TOPRIGHT",nil,"TOPLEFT"}, 75, 105, 0, 16, "^7基底#4:")
...
function ItemsTabClass:AddImplicitToDisplayItem()
	...
	if (self.displayItem.rarity ~= "传奇" and self.displayItem.rarity ~= "遗产") and (self.displayItem.type == "Helmet" or self.displayItem.type == "Body Armour" or self.displayItem.type == "Gloves" or self.displayItem.type == "Boots") then
		...
		for _, compareSlot in pairs(compareSlots) do
			if not main.slotOnlyTooltips or (slot and (slot.nodeId == compareSlot.nodeId or slot.slotName == compareSlot.slotName)) or not slot or slot == compareSlot then
				...
				if item == selItem then
					header = "^7从 "..compareSlot.label.." 移除本装备会让你:"
				else
					header = string.format("^7装备本物品到 %s 会让你:%s", compareSlot.label, selItem and "\n(替换掉 "..colorCodes[selItem.rarity]..selItem.name.."^7)" or "")
				end
				self.build:AddStatComparesToTooltip(tooltip, calcBase, output, header)
			end
