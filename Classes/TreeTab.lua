...
function TreeTabClass:Draw(viewPort, inputEvents)
	...
	for id, spec in ipairs(self.specList) do
		t_insert(newSpecList, (spec.treeVersion ~= latestTreeVersion and ("["..treeVersions[spec.treeVersion].display.."] ") or "")..(spec.title or "默认天赋树"))
	end
...
function TreeTabClass:OpenSpecManagePopup()
	main:OpenPopup(370, 290, "天赋树管理", {
		...
		new("ButtonControl", nil, 0, 260, 90, 20, "完成", function()
			main:ClosePopup()
		end),
	})
end
...
function TreeTabClass:FindTimelessJewel()
	...
	local controls = { }
	local modData = { }
	local ignoredMods = { "瓦尔之力", "瓦尔之遗", "力量", "敏捷", "奉献", "荣耀的代价" }
	...
	local jewelTypes = {
		{ label = "光彩夺目", name = "vaal", id = 1 },
		{ label = "致命的骄傲", name = "karui", id = 2 },
		{ label = "残酷的约束", name = "maraketh", id = 3 },
		{ label = "好战的信仰", name = "templar", id = 4 },
		{ label = "优雅的狂妄", name = "eternal", id = 5 }
	}
	...
	local conquerorTypes = {
		[1] = {
			{ label = "多里亚尼 (腐化的灵魂)", id = 1 },
			{ label = "夏巴夸亚 (神圣血肉)", id = 2 },
			{ label = "阿华纳 (不朽野望)", id = 3 }
		},
		[2] = {
			{ label = "冈姆 (鲜血之力)", id = 1 },
			{ label = "拉凯尔塔 (战争锤炼)", id = 2 },
			{ label = "阿克雅 (破枷除锁)", id = 3 }
		},
		[3] = {
			{ label = "安赛娜丝 (与亡共舞)", id = 1 },
			{ label = "娜斯玛 (慧眼)", id = 2 },
			{ label = "巴巴拉 (叛徒)", id = 3 }
		},
		[4] = {
			{ label = "阿瓦留斯 (决心之力)", id = 1 },
			{ label = "多米纳斯 (内在信念)", id = 2 },
			{ label = "玛萨留斯 (超然飞升)", id = 3 }
		},
		[5] = {
			{ label = "卡迪罗 (无上衰败)", id = 1 },
			{ label = "维多里奥 (无上之秀)", id = 2 },
			{ label = "卡斯皮罗 (无上宣示)", id = 3 }
		}
	}
	...
	controls.jewelSelectLabel = new("LabelControl", { "TOPRIGHT", nil, "TOPLEFT" }, 405, 25, 0, 16, "^7珠宝类型:")
	...
	controls.conquerorSelectLabel = new("LabelControl", { "TOPRIGHT", nil, "TOPLEFT" }, 405, 50, 0, 16, "^7征服者:")
	...
	controls.socketSelectLabel = new("LabelControl", { "TOPRIGHT", nil, "TOPLEFT" }, 405, 75, 0, 16, "^7珠宝孔位置:")
	...
	controls.socketFilter.tooltipFunc = function(tooltip, mode, index, value)
		tooltip:Clear()
		tooltip:AddLine(16, "^7开启该选项后将会排除天赋树中未分配的节点。")
		tooltip:AddLine(16, "^7在你不需要考虑其他天赋点时建议勾选。")
	end
