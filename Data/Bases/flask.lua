-- This file is automatically generated, do not edit!
-- Item data (c) Grinding Gear Games
local itemBases = ...

itemBases["小型生命药剂"] = {
	type = "Flask",
	subType = "Life",
	tags = { flask = true, life_flask = true, default = true, },
	implicitModTypes = { },
	flask = { life = 70, duration = 3, chargesUsed = 7, chargesMax = 21, },
	req = { },
}
itemBases["中型生命药剂"] = {
	type = "Flask",
	subType = "Life",
	tags = { flask = true, life_flask = true, default = true, },
	implicitModTypes = { },
	flask = { life = 150, duration = 3.5, chargesUsed = 8, chargesMax = 28, },
	req = { level = 3, },
}
itemBases["大型生命药剂"] = {
	type = "Flask",
	subType = "Life",
	tags = { flask = true, life_flask = true, default = true, },
	implicitModTypes = { },
	flask = { life = 250, duration = 3.5, chargesUsed = 9, chargesMax = 30, },
	req = { level = 6, },
}
itemBases["良质生命药剂"] = {
	type = "Flask",
	subType = "Life",
	tags = { flask = true, life_flask = true, default = true, },
	implicitModTypes = { },
	flask = { life = 360, duration = 3.5, chargesUsed = 10, chargesMax = 32, },
	req = { level = 12, },
}
itemBases["优质生命药剂"] = {
	type = "Flask",
	subType = "Life",
	tags = { flask = true, life_flask = true, default = true, },
	implicitModTypes = { },
	flask = { life = 640, duration = 3, chargesUsed = 10, chargesMax = 25, },
	req = { level = 18, },
}
itemBases["巨型生命药剂"] = {
	type = "Flask",
	subType = "Life",
	tags = { flask = true, life_flask = true, default = true, },
	implicitModTypes = { },
	flask = { life = 830, duration = 4, chargesUsed = 10, chargesMax = 30, },
	req = { level = 24, },
}
itemBases["高阶生命药剂"] = {
	type = "Flask",
	subType = "Life",
	tags = { flask = true, life_flask = true, default = true, },
	implicitModTypes = { },
	flask = { life = 1000, duration = 3.5, chargesUsed = 10, chargesMax = 32, },
	req = { level = 30, },
}
itemBases["圣语生命药剂"] = {
	type = "Flask",
	subType = "Life",
	tags = { flask = true, life_flask = true, default = true, },
	implicitModTypes = { },
	flask = { life = 1200, duration = 3, chargesUsed = 10, chargesMax = 25, },
	req = { level = 36, },
}
itemBases["祝福生命药剂"] = {
	type = "Flask",
	subType = "Life",
	tags = { flask = true, life_flask = true, default = true, },
	implicitModTypes = { },
	flask = { life = 1990, duration = 4, chargesUsed = 10, chargesMax = 30, },
	req = { level = 42, },
}
itemBases["圣化生命药剂"] = {
	type = "Flask",
	subType = "Life",
	tags = { flask = true, life_flask = true, default = true, },
	implicitModTypes = { },
	flask = { life = 1460, duration = 1.5, chargesUsed = 15, chargesMax = 30, },
	req = { level = 50, },
}
itemBases["不朽生命药剂"] = {
	type = "Flask",
	subType = "Life",
	tags = { flask = true, not_for_sale = true, life_flask = true, default = true, },
	implicitModTypes = { },
	flask = { life = 2400, duration = 3.5, chargesUsed = 15, chargesMax = 45, },
	req = { level = 60, },
}
itemBases["永恒生命药剂"] = {
	type = "Flask",
	subType = "Life",
	tags = { flask = true, not_for_sale = true, life_flask = true, default = true, },
	implicitModTypes = { },
	flask = { life = 2080, duration = 2, chargesUsed = 15, chargesMax = 45, },
	req = { level = 65, },
}

itemBases["小型魔力药剂"] = {
	type = "Flask",
	subType = "Mana",
	tags = { flask = true, mana_flask = true, default = true, },
	implicitModTypes = { },
	flask = { mana = 50, duration = 3, chargesUsed = 6, chargesMax = 24, },
	req = { },
}
itemBases["中型魔力药剂"] = {
	type = "Flask",
	subType = "Mana",
	tags = { flask = true, mana_flask = true, default = true, },
	implicitModTypes = { },
	flask = { mana = 70, duration = 3.5, chargesUsed = 7, chargesMax = 30, },
	req = { level = 3, },
}
itemBases["大型魔力药剂"] = {
	type = "Flask",
	subType = "Mana",
	tags = { flask = true, mana_flask = true, default = true, },
	implicitModTypes = { },
	flask = { mana = 90, duration = 4, chargesUsed = 8, chargesMax = 35, },
	req = { level = 6, },
}
itemBases["良质魔力药剂"] = {
	type = "Flask",
	subType = "Mana",
	tags = { flask = true, mana_flask = true, default = true, },
	implicitModTypes = { },
	flask = { mana = 120, duration = 4, chargesUsed = 9, chargesMax = 36, },
	req = { level = 12, },
}
itemBases["优质魔力药剂"] = {
	type = "Flask",
	subType = "Mana",
	tags = { flask = true, mana_flask = true, default = true, },
	implicitModTypes = { },
	flask = { mana = 170, duration = 4.5, chargesUsed = 7, chargesMax = 30, },
	req = { level = 18, },
}
itemBases["巨型魔力药剂"] = {
	type = "Flask",
	subType = "Mana",
	tags = { flask = true, mana_flask = true, default = true, },
	implicitModTypes = { },
	flask = { mana = 250, duration = 5, chargesUsed = 8, chargesMax = 36, },
	req = { level = 24, },
}
itemBases["高阶魔力药剂"] = {
	type = "Flask",
	subType = "Mana",
	tags = { flask = true, mana_flask = true, default = true, },
	implicitModTypes = { },
	flask = { mana = 350, duration = 5, chargesUsed = 8, chargesMax = 38, },
	req = { level = 30, },
}
itemBases["圣语魔力药剂"] = {
	type = "Flask",
	subType = "Mana",
	tags = { flask = true, mana_flask = true, default = true, },
	implicitModTypes = { },
	flask = { mana = 480, duration = 5.5, chargesUsed = 7, chargesMax = 35, },
	req = { level = 36, },
}
itemBases["祝福魔力药剂"] = {
	type = "Flask",
	subType = "Mana",
	tags = { flask = true, mana_flask = true, default = true, },
	implicitModTypes = { },
	flask = { mana = 700, duration = 6, chargesUsed = 6, chargesMax = 30, },
	req = { level = 42, },
}
itemBases["圣化魔力药剂"] = {
	type = "Flask",
	subType = "Mana",
	tags = { flask = true, mana_flask = true, default = true, },
	implicitModTypes = { },
	flask = { mana = 1100, duration = 6.5, chargesUsed = 7, chargesMax = 35, },
	req = { level = 50, },
}
itemBases["不朽魔力药剂"] = {
	type = "Flask",
	subType = "Mana",
	tags = { flask = true, not_for_sale = true, mana_flask = true, default = true, },
	implicitModTypes = { },
	flask = { mana = 1400, duration = 5, chargesUsed = 8, chargesMax = 42, },
	req = { level = 60, },
}
itemBases["永恒魔力药剂"] = {
	type = "Flask",
	subType = "Mana",
	tags = { flask = true, not_for_sale = true, mana_flask = true, default = true, },
	implicitModTypes = { },
	flask = { mana = 1800, duration = 7, chargesUsed = 8, chargesMax = 42, },
	req = { level = 65, },
}

itemBases["小型复合药剂"] = {
	type = "Flask",
	subType = "Hybrid",
	tags = { flask = true, hybrid_flask = true, default = true, },
	implicitModTypes = { },
	flask = { life = 100, mana = 70, duration = 5, chargesUsed = 20, chargesMax = 40, },
	req = { level = 10, },
}
itemBases["中型复合药剂"] = {
	type = "Flask",
	subType = "Hybrid",
	tags = { flask = true, hybrid_flask = true, default = true, },
	implicitModTypes = { },
	flask = { life = 230, mana = 100, duration = 5, chargesUsed = 20, chargesMax = 40, },
	req = { level = 20, },
}
itemBases["大型复合药剂"] = {
	type = "Flask",
	subType = "Hybrid",
	tags = { flask = true, hybrid_flask = true, default = true, },
	implicitModTypes = { },
	flask = { life = 510, mana = 140, duration = 5, chargesUsed = 20, chargesMax = 40, },
	req = { level = 30, },
}
itemBases["高阶复合药剂"] = {
	type = "Flask",
	subType = "Hybrid",
	tags = { flask = true, hybrid_flask = true, default = true, },
	implicitModTypes = { },
	flask = { life = 690, mana = 200, duration = 5, chargesUsed = 20, chargesMax = 40, },
	req = { level = 40, },
}
itemBases["圣语复合药剂"] = {
	type = "Flask",
	subType = "Hybrid",
	tags = { flask = true, hybrid_flask = true, default = true, },
	implicitModTypes = { },
	flask = { life = 1440, mana = 400, duration = 5, chargesUsed = 20, chargesMax = 40, },
	req = { level = 50, },
}
itemBases["祝福复合药剂"] = {
	type = "Flask",
	subType = "Hybrid",
	tags = { flask = true, hybrid_flask = true, not_for_sale = true, default = true, },
	implicitModTypes = { },
	flask = { life = 1740, mana = 480, duration = 5, chargesUsed = 20, chargesMax = 40, },
	req = { level = 60, },
}

itemBases["宝钻药剂"] = {
	type = "Flask",
	subType = "Utility",
	tags = { flask = true, utility_flask = true, not_for_sale = true, default = true, },
	implicitModTypes = { },
	flask = { duration = 6, chargesUsed = 20, chargesMax = 40, buff = { "攻击和法术暴击率提高 100%" }, },
	req = { level = 27, },
}
itemBases["红玉药剂"] = {
	type = "Flask",
	subType = "Utility",
	tags = { flask = true, utility_flask = true, not_for_sale = true, default = true, },
	implicitModTypes = { },
	flask = { duration = 8, chargesUsed = 20, chargesMax = 50, buff = { "+50% 火焰抗性", "受到的火焰伤害总降 20%" }, },
	req = { level = 18, },
}
itemBases["蓝玉药剂"] = {
	type = "Flask",
	subType = "Utility",
	tags = { flask = true, utility_flask = true, not_for_sale = true, default = true, },
	implicitModTypes = { },
	flask = { duration = 8, chargesUsed = 20, chargesMax = 50, buff = { "+50% 冰霜抗性", "承受的冰霜总伤害降低 20%" }, },
	req = { level = 18, },
}
itemBases["黄玉药剂"] = {
	type = "Flask",
	subType = "Utility",
	tags = { flask = true, utility_flask = true, not_for_sale = true, default = true, },
	implicitModTypes = { },
	flask = { duration = 8, chargesUsed = 20, chargesMax = 50, buff = { "+50% 闪电抗性", "受到的闪电伤害总降 20%" }, },
	req = { level = 18, },
}
itemBases["坚岩药剂"] = {
	type = "Flask",
	subType = "Utility",
	tags = { flask = true, utility_flask = true, not_for_sale = true, default = true, },
	implicitModTypes = { },
	flask = { duration = 6, chargesUsed = 30, chargesMax = 60, buff = { "+1500 护甲" }, },
	req = { level = 27, },
}
itemBases["水银药剂"] = {
	type = "Flask",
	subType = "Utility",
	tags = { flask = true, utility_flask = true, not_for_sale = true, default = true, },
	implicitModTypes = { },
	flask = { duration = 6, chargesUsed = 30, chargesMax = 60, buff = { "移动速度提高 40%" }, },
	req = { level = 4, },
}
itemBases["紫晶药剂"] = {
	type = "Flask",
	subType = "Utility",
	tags = { flask = true, utility_flask = true, not_for_sale = true, default = true, },
	implicitModTypes = { },
	flask = { duration = 6.5, chargesUsed = 35, chargesMax = 65, buff = { "+35% 混沌抗性" }, },
	req = { level = 18, },
}
itemBases["石英药剂"] = {
	type = "Flask",
	subType = "Utility",
	tags = { flask = true, utility_flask = true, not_for_sale = true, default = true, },
	implicitModTypes = { },
	flask = { duration = 6, chargesUsed = 30, chargesMax = 60, buff = { "法术伤害压制率 +10%", "【迷踪】状态" }, },
	req = { level = 27, },
}
itemBases["翠玉药剂"] = {
	type = "Flask",
	subType = "Utility",
	tags = { flask = true, utility_flask = true, not_for_sale = true, default = true, },
	implicitModTypes = { },
	flask = { duration = 6, chargesUsed = 30, chargesMax = 60, buff = { "+1500 点闪避值" }, },
	req = { level = 27, },
}
itemBases["石化药剂"] = {
	type = "Flask",
	subType = "Utility",
	tags = { flask = true, utility_flask = true, not_for_sale = true, default = true, },
	implicit = "使用时嘲讽周围敌人",
	implicitModTypes = { { "flask" }, },
	flask = { duration = 8, chargesUsed = 40, chargesMax = 60, buff = { "护甲总增 20%" }, },
	req = { level = 40, },
}
itemBases["海蓝药剂"] = {
	type = "Flask",
	subType = "Utility",
	tags = { flask = true, utility_flask = true, not_for_sale = true, default = true, },
	implicit = "使用时产生冰缓地面",
	implicitModTypes = { { "flask" }, },
	flask = { duration = 8, chargesUsed = 20, chargesMax = 50, buff = { "減少 60% 你身上的冰冻效果" }, },
	req = { level = 27, },
}
itemBases["迷雾药剂"] = {
	type = "Flask",
	subType = "Utility",
	tags = { flask = true, utility_flask = true, not_for_sale = true, default = true, },
	implicit = "使用时产生一片烟雾",
	implicitModTypes = { { "flask" }, },
	flask = { duration = 8, chargesUsed = 40, chargesMax = 60, buff = { "闪避值总增 20%" }, },
	req = { level = 40, },
}
itemBases["硫磺药剂"] = {
	type = "Flask",
	subType = "Utility",
	tags = { flask = true, utility_flask = true, not_for_sale = true, default = true, },
	implicit = "使用时产生奉献地面",
	implicitModTypes = { { "flask" }, },
	flask = { duration = 8, chargesUsed = 40, chargesMax = 60, buff = { "伤害提高 40%" }, },
	req = { level = 35, },
}
itemBases["真银药剂"] = {
	type = "Flask",
	subType = "Utility",
	tags = { flask = true, utility_flask = true, not_for_sale = true, no_effect_flask_mod = true, default = true, },
	implicitModTypes = { },
	flask = { duration = 6, chargesUsed = 40, chargesMax = 60, buff = { "猛攻" }, },
	req = { level = 40, },
}
itemBases["灰岩药剂"] = {
	type = "Flask",
	subType = "Utility",
	tags = { flask = true, utility_flask = true, not_for_sale = true, default = true, },
	implicitModTypes = { },
	flask = { duration = 8.5, chargesUsed = 15, chargesMax = 40, buff = { "+35% 所有元素抗性" }, },
	req = { level = 8, },
}
itemBases["黄金药剂"] = {
	type = "Flask",
	subType = "Utility",
	tags = { flask = true, utility_flask = true, not_for_sale = true, default = true, },
	implicitModTypes = { },
	flask = { duration = 5, chargesUsed = 60, chargesMax = 80, buff = { "物品稀有度提高 30%" }, },
	req = { level = 22, },
}
itemBases["刚玉药剂"] = {
	type = "Flask",
	subType = "Utility",
	tags = { flask = true, utility_flask = true, not_for_sale = true, default = true, },
	implicitModTypes = { },
	flask = { duration = 7.5, chargesUsed = 20, chargesMax = 50, buff = { "晕眩门槛提高 30%" }, },
	req = { level = 27, },
}
itemBases["坚铁药剂"] = {
	type = "Flask",
	subType = "Utility",
	tags = { flask = true, utility_flask = true, not_for_sale = true, expedition_flask = true, default = true, },
	implicit = "使用时恢复结界",
	implicitModTypes = { { "flask" }, },
	flask = { duration = 5, chargesUsed = 40, chargesMax = 60, buff = { "结界 +200" }, },
	req = { level = 40, },
}
